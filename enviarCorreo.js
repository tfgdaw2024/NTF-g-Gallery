function enviarCorreoBienvenida() {
  ...
  filas.forEach((fila, index) => {
    ...
    if (fila[10] !== "Sí") { // Suponiendo que la columna 'Emails Sent' está en la columna 11 (índice 10)
      var nombre = fila[3]; // Suponiendo que 'User Nicename' es la cuarta columna (índice 3)
      var email = fila[4]; // Suponiendo que 'User Email' es la quinta columna (índice 4)
      ...
      if (email) {
        ...
        MailApp.sendEmail({
          ...
          body: 'Se ha registrado un nuevo usuario: ' + nombre + ' (' + email + ').'
        });
        ...
        console.log('No se ha enviado el correo porque el campo de email está vacío para la fila ' + (index + 2));
      }
    }
  });
}
